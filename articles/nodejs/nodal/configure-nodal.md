# Configure Nodal for Production

## Setup a webserver
Nodal runs best in production with a reverse-proxy setup. Let's configure nginx to serve static assets directly, handle compression, and proxy connections into nodal through node's builtin server.
...
http {
    ...
    server {
        ...
        try_files $uri/index.html $uri @nodal;

        # Proxy connections to nodal
        location @nodal {
            proxy_pass         http://nodal;
            proxy_redirect     off;
...
## Compile Assets
We can have nodal compile assets during the deploy process by adding an extra step:
...
* [Preview your App](/nodejs/nodal/preview-your-app)
* [Launch your App](/nodejs/nodal/launch-your-app)
