# Configure Pyramid for Production
...
## Setup a webserver
Pyramid runs best in production with a reverse-proxy setup. Let's configure nginx to serve static assets directly, handle compression, and proxy connections into Pyramid through gunicorn.
...
web.main:
  start:
    ...
    pyramid: gunicorn -c /app/etc/gunicorn.py YOUR_DJANGO_APP.wsgi
```
...
For Pyramid to run behind nginx in production, we'll need to ensure the assets get collected from all of the Pyramid apps and dropped into the `assets` directory (which was where we told nginx to serve them from).
...
* [Preview your App](/python/pyramid/preview-your-app)
* [Launch your App](/python/pyramid/launch-your-app)
